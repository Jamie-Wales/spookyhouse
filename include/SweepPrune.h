...
struct sweepObject {
    ...
    sweepObject(int id, std::shared_ptr<Mesh> mesh)
        : id(id)
        , mesh(mesh) {};
};
...
public:
    ...
    void printTrueCollisions(std::vector<BroadCollision>& trueCol)
    {
    ...
    std::vector<BroadCollision> getTrueCollisions()
    {
        ...
        for (const auto& entry : collisionMap) {
            ...
            int firstModelId = modToId[firstId]->id;
            int secondModelId = modToId[secondId]->id;
            if (entry.second.size() == 3 && firstId != secondId && firstModelId != secondModelId) {
                trueCollisions.push_back(BroadCollision(firstModelId, secondModelId, sweepMap[firstId]->mesh, sweepMap[secondId]->mesh));
            }
        }
