...
public:
    ...
    void UpdateObject(std::shared_ptr<Model> model) {
        ...
        for (auto &sweep: vec) {
            ...
            if (sweep->mesh->boundingbox.yaw != model->yaw || sweep->mesh->boundingbox.pitch != model->pitch)
                bb.rotate(sweep->mesh->boundingbox.pitch- bb.pitch, sweep->mesh->boundingbox.yaw - bb.yaw, 0.0);
